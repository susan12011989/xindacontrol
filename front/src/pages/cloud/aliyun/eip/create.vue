...
import type { CreateEipItemData } from "@/pages/cloud/aliyun/eip/apis/type"
...
import { batchCreateEip } from "@/pages/cloud/aliyun/eip/apis"
...
// 数据状态
const accountType = ref<string>("merchant") // merchant/system
...
// EIP列表
const eipList = ref<CreateEipItemData[]>([])
...
// 创建新EIP配置
function createDefaultEip(): CreateEipItemData {
  return {
    ...
    region_id: "",
    instance_charge_type: "PostPaid", // 默认按量付费
    internet_charge_type: "PayByTraffic", // 默认按流量计费
    bandwidth: "1", // 默认带宽1Mbps
    num: 1 // 默认数量1
  }
}
...
function addEip() {
  ...
  if (regionList.value.length === 0) {
    ...
    fetchRegionList().then(() => {
      if (regionList.value.length > 0) {
        eipList.value.unshift(createDefaultEip())
      } else {
        ElMessage.error("区域列表为空，无法添加EIP")
      }
    })
    return
  }
...
// 移除一个EIP配置
function removeEip(index: number) {
  eipList.value.splice(index, 1)
}
...
const canSubmit = computed(() => {
  if ((accountType.value === "merchant" && !selectedMerchant.value)
    ...
    || eipList.value.length === 0) {
    return false
  }

  // 检查所有EIP配置是否都已填写必填项
  return eipList.value.every(eip =>
    eip.region_id // 必须选择区域
    && Number(eip.bandwidth) > 0 // 带宽必须大于0
    && Number(eip.num) > 0 // 数量必须大于0
  )
})

// 取消流式请求的函数引用
let cancelCreateEip: (() => void) | null = null

// 提交创建EIP
async function submitCreate() {
  if (!canSubmit.value) {
    ElMessage.warning("请完善所有EIP配置")
    return
  }

  // 确保所有EIP都设置了最新的账号ID
  eipList.value.forEach((eip) => {
    if (accountType.value === "merchant") {
      eip.merchant_id = selectedMerchant.value || 0
      delete eip.cloud_account_id
    } else {
      eip.cloud_account_id = selectedCloudAccount.value || 0
      delete eip.merchant_id
    }
  })
  ...
  try {
    ...
    cancelCreateEip = null

    console.log("开始创建EIP列表:", eipList.value)

    // 使用流式API
    cancelCreateEip = batchCreateEip(
      {
        list: [...eipList.value]
      },
      ...
      (error) => {
        console.error("创建EIP失败:", error)
        createLoading.value = false
        cancelCreateEip = null
        ElMessage.error("创建EIP失败")
      }
    ) as (() => void) | null
  } catch (error) {
    console.error("创建EIP失败:", error)
...
function handleDialogClose() {
  if (createCompleted.value) {
    // 创建完成后返回EIP列表页面
    router.push("/cloud/aliyun/eip")
  } else if (cancelCreateEip) {
    // 如果还在创建，询问是否取消
    ElMessageBox.confirm(
      "取消创建将停止处理，已创建的EIP不会被删除。确定取消吗？",
      "取消确认",
      {
  ...
  <div class="container">
    ...
    <div class="page-nav">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/cloud/aliyun/eip' }">
          弹性IP列表
        </el-breadcrumb-item>
        <el-breadcrumb-item>创建弹性IP</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      <div class="header-row">
        <div class="select-container">
          <div class="select-item">
            ...
            <el-select v-model="accountType" style="width: 150px" @change="() => { selectedMerchant = undefined as any; selectedCloudAccount = undefined as any; eipList = [] as any }">
    ...
    <!-- EIP列表 -->
    <el-card class="eip-card" v-if="eipList.length > 0" shadow="hover">
      <div v-for="(eip, index) in eipList" :key="index" class="eip-item">
        <div class="eip-header">
          <el-tag size="large" type="info" effect="plain" class="eip-tag">
            弹性IP #{{ eipList.length - index }}
          </el-tag>
          <el-button type="danger" link @click="removeEip(index)" :disabled="createLoading">
          ...
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="区域" required>
                <el-select v-model="eip.region_id" placeholder="请选择区域" filterable style="width: 100%">
                  <template #prefix>
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="计费类型" required>
                <el-radio-group v-model="eip.instance_charge_type">
                  <el-radio label="PostPaid">
                    按量付费
                  </el-radio>
                  <el-radio label="PrePaid">
                    包年包月
                  </el-radio>
                </el-radio-group>
              </el-form-item>
            ...
            <el-col :span="8">
              <el-form-item label="网络计费方式" required>
                <el-radio-group v-model="eip.internet_charge_type">
                  <el-radio label="PayByTraffic">
                    按流量计费
                  </el-radio>
                  <el-radio label="PayByBandwidth">
                    按带宽计费
                  </el-radio>
                </el-radio-group>
              </el-form-item>
            ...
            <el-col :span="4">
              <el-form-item label="带宽" required>
                <el-input v-model="eip.bandwidth" placeholder="请输入带宽值" type="number" min="1" max="200">
                  <template #append>
                    Mbps
                  </template>
                </el-input>
            ...
            <el-col :span="4">
              <el-form-item label="数量" required>
                <el-input v-model.number="eip.num" placeholder="请输入数量" type="number" min="1" max="100">
                  <template #append>
                    个
                  </template>
                </el-input>
        ...
        <el-divider v-if="index < eipList.length - 1" />
      </div>
    ...
    <el-empty v-else-if="selectedMerchant" description="请添加弹性IP配置" />

    <!-- 创建进度对话框 -->
    <el-dialog
      v-model="progressDialogVisible"
      :title="createCompleted ? '创建完成' : '弹性IP创建进度'"
      width="700px"
      ...
      <div class="progress-dialog-content">
        ...
        <div v-if="createCompleted" class="completion-message">
          <el-alert
            title="创建完成"
            type="success"
            :description="completionMessage || '已成功创建所有弹性IP'"
            show-icon
          />
        </div>
      </div>
