...
/**
 * 阿里云 OSS 对象存储管理
 * 参考 AWS S3 设计，单页面管理 Bucket 和对象
 */
import type { OssObjectItem } from "@/pages/cloud/aliyun/oss/apis/type"
...
import { downloadObject, listBuckets, listObjects, uploadObject } from "@/pages/cloud/aliyun/oss/apis"
...
function selectBucket(bucket: { name: string, location: string }) {
  ...
  // 阿里云返回的location格式为 oss-cn-hangzhou，需要去掉 oss- 前缀
  form.region_id = bucket.location.replace(/^oss-/, "")
  onListObjects()
}
...
// ========== 下载操作 ==========
async function onDownload(row: OssObjectItem) {
...
// ========== 删除操作 ==========
async function onDelete(row: OssObjectItem) {
  if (!form.region_id || !form.bucket) return
  ...
  try {
    ...
    await aliyunDeleteObject(params)
...
// ========== 复制链接 ==========
async function onCopyUrl(row: OssObjectItem) {
  try {
    const url = generateObjectUrl("aliyun", form.bucket, row.key, form.region_id)
...
function formatStorageClass(storageClass: string) {
  const map: Record<string, { label: string, type: string }> = {
    ...
    IA: { label: "低频访问", type: "warning" },
    Archive: { label: "归档存储", type: "info" },
    ColdArchive: { label: "冷归档", type: "info" }
  }
  return map[storageClass] || { label: storageClass, type: "" }
}
...
function onShowCreateBucket() {
  ...
  createBucketForm.storage_class = "Standard"
...
async function onCreateBucket() {
  ...
  try {
    const params: any = {
      ...
      storage_class: createBucketForm.storage_class
    }
    if (accountType.value === "merchant") {
      params.merchant_id = selectedMerchant.value
    } else {
    ...
    await aliyunCreateBucket(params)
...
async function onDeleteBucket(bucket: { name: string }) {
  ...
  try {
    ...
    await aliyunDeleteBucket(params)
    ElMessage.success("删除成功")
...
async function onSetBucketPublic(command: string) {
  ...
  try {
    ...
    await aliyunSetBucketPublic(params)
  ...
  <div class="app-container">
    ...
    <div class="page-header">
      <div class="header-left">
        <h2 class="title">阿里云 OSS 对象存储</h2>
        <p class="subtitle">管理您的 OSS Buckets 和对象文件</p>
      </div>
    </div>
    ...
    <el-card v-if="buckets.length || (accountType === 'system' ? selectedCloudAccount : selectedMerchant)" shadow="hover" class="mb-4 bucket-list-card">
      ...
      <div v-if="buckets.length" class="bucket-grid">
        ...
        >
          ...
          <div class="bucket-region">{{ bucket.location }}</div>
          <div class="bucket-storage">
            <el-tag size="small" :type="formatStorageClass(bucket.storage_class).type as any">
              {{ formatStorageClass(bucket.storage_class).label }}
            </el-tag>
          </div>
    ...
    <el-dialog v-model="createBucketDialog" title="创建 Bucket" width="500px">
      <el-form label-position="top">
        ...
        <el-form-item label="存储类型">
          <el-select v-model="createBucketForm.storage_class" style="width: 100%">
            <el-option label="标准存储 (Standard)" value="Standard" />
            <el-option label="低频访问 (IA)" value="IA" />
            <el-option label="归档存储 (Archive)" value="Archive" />
          </el-select>
        </el-form-item>
