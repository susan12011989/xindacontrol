...
func UploadServiceFile(serverId int, serviceName string, filename string, reader io.Reader) (string, error) {
	...
	client.ExecuteCommand(fmt.Sprintf("if [ -f '%s' ]; then sudo cp -f '%s' '%s'; fi", remotePath, remotePath, backupPath))

	// 上传到 /tmp/（避免目标目录权限问题），然后 sudo mv 到目标路径
	tmpPath := fmt.Sprintf("/tmp/deploy_%s_%s.tmp", binaryName, ts)
	if err = client.UploadFile(tmpPath, reader); err != nil {
		return "", fmt.Errorf("上传文件失败: %v", err)
	}

	// 原子替换（使用 sudo 避免权限问题）
	if _, err = client.ExecuteCommand(fmt.Sprintf("sudo mv -f '%s' '%s'", tmpPath, remotePath)); err != nil {
		return "", fmt.Errorf("替换文件失败: %v", err)
	}

	// 设置可执行权限
	client.ExecuteCommand(fmt.Sprintf("sudo chmod +x '%s'", remotePath))
...
func DistributeFile(req model.DistributeFileReq, operator string) (model.DistributeFileResp, error) {
	...
	for _, target := range targetServers {
		...
		// 确保远程目录存在
		targetClient.ExecuteCommand(fmt.Sprintf("sudo mkdir -p %s", remoteUploadPath))

		// 备份远程原文件
		ts := time.Now().Format("20060102_150405")
		backupPath := fmt.Sprintf("%s.%s.bak", remoteFilePath, ts)
		targetClient.ExecuteCommand(fmt.Sprintf("if [ -f '%s' ]; then sudo cp -f '%s' '%s'; fi", remoteFilePath, remoteFilePath, backupPath))
		...
		// 上传到 /tmp/（ubuntu 用户有写权限），然后 sudo mv 到目标路径
		tmpPath := fmt.Sprintf("/tmp/deploy_%s_%s.tmp", binaryName, ts)
		...
		// 原子替换（使用 sudo 避免权限问题）
		_, mvErr := targetClient.ExecuteCommand(fmt.Sprintf("sudo mv -f '%s' '%s'", tmpPath, remoteFilePath))
		if mvErr != nil {
			result.Success = false
		...
		// 设置可执行权限
		targetClient.ExecuteCommand(fmt.Sprintf("sudo chmod +x '%s'", remoteFilePath))

		// 如果需要重启服务
		if req.RestartAfter {
			dockerName := model.ServiceDockerNames[req.ServiceName]

			// docker-compose.yml 所在目录（与二进制上传目录可能不同）
			composePath := model.ServiceComposePaths[req.ServiceName]
			if composePath == "" {
				composePath = remoteUploadPath // 兜底
			}
			// 计算二进制相对于 compose 目录的路径（用于 volume 挂载）
			relBinaryPath := strings.TrimPrefix(remoteFilePath, composePath)
...
if [ "$NEEDS_RECREATE" = "1" ]; then
  sudo docker compose up -d %s 2>&1 || sudo docker-compose up -d %s 2>&1
else
  sudo docker restart %s 2>&1
fi
`, composePath,
				req.ServiceName,
				relBinaryPath,
...
`, composeContent, envContent)
		...
		if err != nil {
			...
			// App 节点同步成功后，重新应用 Web 品牌配置（logo/应用名称）
			if (node.NodeRole == "app" || node.NodeRole == "") && AfterAppDeployHook != nil && node.MerchantId > 0 {
				go func(mid int) {
					logx.Infof("[BatchSyncConfig] 重新应用 Web 品牌配置: merchantId=%d", mid)
					AfterAppDeployHook(mid)
				}(node.MerchantId)
			}
		}
